#Write a program to implement a Turing  Machine for string of even length over Σ = {0,1}. 
...
def even_length_stack_tm(input_str):
    tape = list(input_str)
    head = 0
    symbols = {"0", "1"}
    stack = []
    ...
    # Move head across the tape and push symbols onto stack
    while head < len(tape):
    ...
    if len(tape) % 2 == 0:
        print("Length is even → accept")
        return True
    else:
        print("Length is odd → reject")
        return False
...
while True:
    ...
    accepted = even_length_stack_tm(s)
